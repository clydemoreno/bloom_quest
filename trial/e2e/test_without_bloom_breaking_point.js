...
const initialVUS = __ENV.VUS || 100;
let VUS = initialVUS;
...
// Define the maximum acceptable P90 response time threshold (in milliseconds)
const maxP90ResponseTimeThreshold = 10000; // 10,000 milliseconds (10 seconds)

export const options = {
  stages: [
    { duration: '10s', target: VUS * 0.50 }, // Ramp up to the specified number of VUs over 10 seconds
    { duration: '30m', target: VUS }, // Stay at the specified number of VUs for 30 minutes (usebloom=0)
...
export default function () {
  ...
  if (__VU <= VUS) {
    ...
    check(responseWithoutBloom, {
      ...
      'response time is within threshold': (r) => r.timings.duration <= maxP90ResponseTimeThreshold,
    });
    ...
    // Check if the P90 response time exceeds the threshold, and stop making requests if it does
    if (responseWithoutBloom.timings.duration.p90 > maxP90ResponseTimeThreshold) {
      console.warn(`P90 response time exceeded the threshold (${maxP90ResponseTimeThreshold} ms). Stopping further requests.`);
      return;
    }

    // If we haven't reached the target VUs, slowly increase by 10%
    if (VUS < initialVUS) {
      VUS += Math.ceil(initialVUS * 0.1);
    }
...
export function teardown(data) {
  ...
  // console.log(`- P90 Response Time: ${responseTimesWithoutBloom.percentiles['90']} ms`);
